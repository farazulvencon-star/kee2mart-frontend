...
import {
  ...
  getUncoveredRequiredVariantIds,
} from "@/utils/prescriptionGroups";
...
const CartDrawer = ({ showCart, setShowCart, setMobileActiveKey }) => {
  ...
  const prescriptionGroups = useSelector(
    (state) => state.Checkout.prescriptionGroups
  );

  const [showLogin, setShowLogin] = useState(false);
  // When a guest logs in via the "Login to checkout" button, send them to the
  // cart page afterwards (instead of leaving them on the home page).
  const [redirectToCartOnLogin, setRedirectToCartOnLogin] = useState(false);
  ...
  // Refresh cart (and the header count badge) when the module changes,
  ...
  // After a guest logs in from "Login to checkout", take them to the cart page.
  useEffect(() => {
    if (redirectToCartOnLogin && user?.jwtToken) {
      setRedirectToCartOnLogin(false);
      setShowLogin(false);
      router.push("/cart");
    }
  }, [user?.jwtToken, redirectToCartOnLogin]);
  ...
  const handleCheckoutbtnClick = () => {
    // Guests upload the prescription AFTER login, so don't block them on the
    // prescription check here — send them straight to login.
    if (cart.isGuest) {
      setShowCart(false);
      setShowLogin(true);
      setRedirectToCartOnLogin(true); // go to cart page after they log in
      return;
    }
    const requiresPrescription = cartProductsData?.some(
      (p) => p?.require_prescription == 1
    );
    const uncovered = getUncoveredRequiredVariantIds(
      cartProductsData,
      prescriptionGroups
    );
    if (requiresPrescription && uncovered.length > 0) {
      toast.error(t("prescription_required_message"));
      return;
    }

...
import { isRtl, resolveCardBg } from '@/lib/utils';
const BrandSlider = ({ brands }) => {
    ...
    const bgMode = brands?.brand_background_color_mode
    const bgGlobalLight = brands?.brand_background_color_light_theme
    const bgGlobalDark = brands?.brand_background_color_dark_theme
    const getBrandBg = (brand) => resolveCardBg({ mode: bgMode, item: brand, globalLight: bgGlobalLight, globalDark: bgGlobalDark, isDark })
                        ...
                        >
                            {brands?.brands?.map((brand, index) => (
                                <SwiperSlide key={index} onClick={() => handleBrandClick(brand)}>
                                    <BrandCard brand={brand} bgColor={getBrandBg(brand)} />
                                </SwiperSlide>
                            ))}
                        </Swiper>
                    </div>
                    {/* Mobile: 2-column grid */}
                    <div className='grid grid-cols-4 gap-3 justify-items-center md:hidden'>
                        {brands?.brands?.slice(0, 8).map((brand, index) => (
                            <div className='w-full flex justify-center' key={index} onClick={() => handleBrandClick(brand)}>
                                <BrandCard brand={brand} bgColor={getBrandBg(brand)} />
                            </div>
                        ))}
                    </div>

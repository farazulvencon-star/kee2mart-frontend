...
import { isRtl, formatPriceDisplay } from "@/lib/utils";
  ...
  return (
    <div>
      <LocalizedLink
        ...
        className="flex flex-col gap-3 group max-w-[170px]  textColor"
      >
        {/* Image */}
        <div className="relative self-stretch max-h-[170px] max-w-[170px]  aspect-square w-full rounded-2xl overflow-hidden">
          ...
          <ul className="absolute right-3 top-3 lg:flex flex-col gap-2 translate-x-3 group-hover:translate-x-0 opacity-0 group-hover:opacity-100 transition-all duration-500 ease-in-out hidden">
            <li
            ...
            {!isProductAvailabel ? (
              isProductAlreadyAdded ? (
                ...
                >
                  ...
                  <span className="px-2 py-0.5 bg-white border-l rounded-b-lg border-r border-b primaryColorBorder primaryColor text-[10px] font-bold text-center leading-tight">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* <button
              onClick={(e) => handleShowVariantModal(e, product)}
              className="px-2 py-0.5 buttonBackground rounded-full text-xs font-normal text-zinc-900 flex items-center gap-1"
            >
              {`${selectedVariant?.measurement} ${selectedVariant?.unit?.translations?.short_code ?? selectedVariant?.unit?.short_code}`}
              {productsVariants?.length > 1 ? <MdArrowDropDown size={14} /> : null}
            </button> */}
            <div className="textColor text-xs font-medium">
                {`${selectedVariant?.measurement} ${selectedVariant?.unit?.translations?.short_code ?? selectedVariant?.unit?.short_code}`}
            </div>
          </div>

          {/* title */}
          <h3 className="textColor text-base font-bold leading-6 line-clamp-2 capitalize group-hover:primaryColor">
              ...
              <div className="flex items-center gap-2">
                ...
                selectedVariant?.discounted_price !== selectedVariant?.price ? (
                  <>
                    ...
                    <span className="opacity-40 textColor text-sm font-medium line-through">
                      {setting?.currency}
